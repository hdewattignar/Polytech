...
namespace Predicate
{
    public partial class Form1 : Form
    {
        ...
        public bool isEven(int inputInteger)
        {
            bool isAnEven = ((inputInteger % 2) == 0);
            return isAnEven;
        }

        public bool isLessThanTen(int inputInteger)
        {
            bool isLess = inputInteger < 10;
            return isLess;
        }
        ...
        private void btn_EvenNumbers_Click(object sender, EventArgs e)
        {
            ...
            predicate = new Predicate<int>(isEven);
            for (int i = 0; i < randomNumbers.Count; i++)
            {
        ...
        {
            listBox_Sorted.Items.Clear();
            predicate = new Predicate<int>(isLessThanTen);
            for (int i = 0; i < randomNumbers.Count; i++)
            {
                if (predicate(randomNumbers[i]))
                {
